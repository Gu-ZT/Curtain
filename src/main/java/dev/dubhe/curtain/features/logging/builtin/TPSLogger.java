...
public class TPSLogger extends AbstractHudLogger {
    ...
    public Component display(ServerPlayer player) {
        ...
        if (averageTPS.isEmpty()) {
            return new TextComponent("No TPS data available").withStyle(ChatFormatting.RED);
        }
        ...
        return new TextComponent("TPS: ").withStyle(ChatFormatting.GRAY)
                .append(new TextComponent("%.1f".formatted(TPS)).withStyle(finalColor))
                .append(new TextComponent(" MSPT: ").withStyle(ChatFormatting.GRAY))
                .append(new TextComponent("%.1f".formatted(MSPT)).withStyle(finalColor));
    }
}
