...
import dev.dubhe.curtain.network.ServerNetworkHandler;
...
public class TickCommand {
    ...
    private static int setWarp(CommandSourceStack source, int advance, String tail_command)
    {
        ...
        @Nullable ServerPlayer player = (entity instanceof ServerPlayer) ? (ServerPlayer) entity : null; // may be null
        ServerTickRateManager trm = ((MinecraftServerInterface)source.getServer()).getTickRateManager();
    ...
    {
        CurtainProfiler.prepare_tick_report(source, ticks);
        return 1;
    }

    public static int healthEntities(CommandSourceStack source, int ticks)
    {
        CurtainProfiler.prepare_entity_report(source, ticks);
        return 1;
    }
